...
class TblPermohonanController extends Controller
{
    ...
    public function actionView($id)
    {
        ...
        $query ->select([
            ...
            'tbl_cawangan.cawangan_nama as cawanganNama',
            'tbl_statuspermohonan.statusPermohonan_nama as statusPermohonan',
            ...
            'tbl_perkara.perkara_pegawaiJawatan as jawatan'
               ...
               ->from('tbl_perkara','tbl_permohonan','tbl_cawangan','tbl_statuspermohonan')
               ...
               ->innerJoin('tbl_cawangan','tbl_permohonan.cawangan_id=tbl_cawangan.cawangan_id')
               ->innerJoin('tbl_statuspermohonan','tbl_permohonan.statusPermohonan_id=tbl_statuspermohonan.statusPermohonan_id')
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $oldIDs = ArrayHelper::map($modelsPerkara, 'perkara_id', 'perkara_id');
            $modelsPerkara = \frontend\models\Model::createMultiple(TblPerkara::classname(), $modelsPerkara);
            Model::loadMultiple($modelsPerkara, Yii::$app->request->post());
            $deletedIDs = array_diff($oldIDs, array_filter(ArrayHelper::map($modelsPerkara, 'perkara_id', 'perkara_id')));
            ...
            if ($valid) {
                ...
                try {
                    if ($flag = $model->save(false)) {
                        if (! empty($deletedIDs)) {
                            TblPerkara::deleteAll(['perkara_id' => $deletedIDs]);
                        }
                        foreach ($modelsPerkara as $modelPerkara) {
                            $modelPerkara->permohonan_id = $model->permohonan_id;
                            if (! ($flag = $modelPerkara->save(false))) {
